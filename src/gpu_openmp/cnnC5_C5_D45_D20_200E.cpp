...
struct Activation {
    static inline double apply(int type, double x) {
        if (type == 0) return 1.0 / (1.0 + std::exp(-x)); // Sigmoid
        if (type == 1) return x > 0 ? x : 0;              // ReLU
        return x;
    }
    static inline double derivative(int type, double output) {
        if (type == 0) return output * (1.0 - output);
        if (type == 1) return output > 0 ? 1.0 : 0.0;
        return 1.0;
    }
};
...
public:
    ...
    Layer(const Params& p)
        : type(p.type), input_size(p.input_size), output_size(p.output_size),
          learning_rate(p.learning_rate), l2_lambda(p.l2_lambda), activation_type(p.activation_type),
    ...
    ~Layer() { delete[] weights; delete[] biases; }

    void forward(double* input_buf, double* output_buf, int batch_size) {
        if (type == 1) {
            #pragma omp target teams distribute parallel for collapse(2) \
                map(to: input_buf[0:batch_size*input_size], weights[0:input_size*output_size], biases[0:output_size]) \
                map(from: output_buf[0:batch_size*output_size])
            ...
            #pragma omp target teams distribute parallel for collapse(2) \
                map(to: input_buf[0:batch_size*input_size], weights[0:kernel_size], biases[0:1]) \
                map(from: output_buf[0:batch_size*output_size])
            for (int b = 0; b < batch_size; ++b) {
    ...
    void calculate_delta(double* this_layer_delta, double* this_layer_output,
                         double* next_layer_delta, double* next_layer_weights,
                         int next_layer_output_size, int batch_size) {
        #pragma omp target teams distribute parallel for collapse(2) \
            map(to: this_layer_output[0:batch_size*output_size], \
                    next_layer_delta[0:batch_size*next_layer_output_size], \
                    next_layer_weights[0:output_size*next_layer_output_size]) \
            map(from: this_layer_delta[0:batch_size*output_size])
        for (int b = 0; b < batch_size; ++b) {
            for (int j = 0; j < output_size; ++j) {
                double error = 0.0;
                for (int k = 0; k < next_layer_output_size; ++k)
                    error += next_layer_delta[b * next_layer_output_size + k] * next_layer_weights[k * output_size + j];
                double current_output = this_layer_output[b * output_size + j];
                this_layer_delta[b * output_size + j] = error * Activation::derivative(activation_type, current_output);
            }
        }
    }

    void update_parameters(double* prev_layer_output, double* this_layer_delta, int batch_size) {
        if (type == 1) {
            #pragma omp target teams distribute parallel for \
                map(to: prev_layer_output[0:batch_size*input_size], this_layer_delta[0:batch_size*output_size]) \
                map(tofrom: weights[0:input_size*output_size], biases[0:output_size])
            for (int j = 0; j < output_size; ++j) {
                for (int i = 0; i < input_size; ++i) {
                    double grad = 0.0;
                    for (int b = 0; b < batch_size; ++b)
                        grad += this_layer_delta[b * output_size + j] * prev_layer_output[b * input_size + i];
                    grad = (grad / batch_size) + l2_lambda * weights[j * input_size + i];
                    weights[j * input_size + i] -= learning_rate * grad;
                }
                double bias_grad = 0.0;
                for (int b = 0; b < batch_size; ++b) bias_grad += this_layer_delta[b * output_size + j];
                biases[j] -= learning_rate * (bias_grad / batch_size);
            }
        } else {
            #pragma omp target teams distribute parallel for \
                map(to: prev_layer_output[0:batch_size*input_size], this_layer_delta[0:batch_size*output_size]) \
                map(tofrom: weights[0:kernel_size], biases[0:1])
            for (int k = 0; k < kernel_size; ++k) {
                double grad = 0.0;
                for (int b = 0; b < batch_size; ++b) {
                    for (int i = 0; i < output_size; ++i) {
                        ...
                        if (in_idx >= 0 && in_idx < input_size)
                            grad += prev_layer_output[b * input_size + in_idx] * this_layer_delta[b * output_size + i];
                    }
                }
            ...
            for (int b = 0; b < batch_size; ++b)
                for (int i = 0; i < output_size; ++i)
                    bias_grad += this_layer_delta[b * output_size + i];
...
public:
    ...
    std::vector<double*> layer_deltas;

    ~NeuralNetwork() {
        for (auto layer : layers) delete layer;
        for (auto out : layer_outputs) delete[] out;
        for (auto delta : layer_deltas) delete[] delta;
    }
    ...
    double* predict(double* inputs, int num_samples, int num_features, int batch_size) {
        ...
        for (int i = 0; i < num_samples; i += batch_size) {
            ...
            for(int b = 0; b < current_batch_size; ++b) {
                for(int j=0; j < layers.back()->output_size; ++j) {
                    all_predictions[(i + b) * layers.back()->output_size + j] = layer_outputs.back()[b * layers.back()->output_size + j];
                }
            }
        }
    ...
    void train(double* inputs, double* labels, int num_samples, int num_features, int num_classes, int epochs, int batch_size) {
        std::cout << "Starting OMP training..." << std::endl;

        for (const auto& layer : layers) {
            layer_outputs.push_back(new double[batch_size * layer->output_size]);
            layer_deltas.push_back(new double[batch_size * layer->output_size]);
        }
            ...
            for (int i = 0; i < num_samples; i += batch_size) {
                ...
                for (int b = 0; b < current_batch_size; ++b) {
                    for (int j = 0; j < num_classes; ++j) {
                        layer_deltas.back()[b * num_classes + j] = layer_outputs.back()[b * num_classes + j] - batch_labels[b * num_classes + j];
                    }
                }

                for (int l = layers.size() - 2; l >= 0; --l) {
                    layers[l]->calculate_delta(layer_deltas[l], layer_outputs[l],
                                             layer_deltas[l+1], layers[l+1]->weights,
                                             layers[l+1]->output_size, current_batch_size);
                }
                
                for (size_t l = 0; l < layers.size(); ++l) {
                    double* prev_layer_output = (l > 0) ? layer_outputs[l-1] : batch_inputs;
                    layers[l]->update_parameters(prev_layer_output, layer_deltas[l], current_batch_size);
                }
            }
             if ((e + 1) % 10 == 0) {
