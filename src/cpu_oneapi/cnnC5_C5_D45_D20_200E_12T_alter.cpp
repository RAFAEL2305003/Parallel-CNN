...
public:
          ...
          d_delta(sycl::range<1>(1)), d_input_delta(sycl::range<1>(1))
    {
        ...
        if (type == 1) {
            h_weights.resize(input_size * output_size);
            h_biases.resize(output_size);
            ...
            d_weights = sycl::buffer<double>(h_weights.data(), sycl::range<1>(input_size * output_size));
            d_biases = sycl::buffer<double>(h_biases.data(), sycl::range<1>(output_size));
            std::cout << "Dense Layer created: " << input_size << " -> " << output_size << std::endl;

        } else if (type == 0) {
            this->output_size = ((input_size + 2 * padding - kernel_size) / stride) + 1;
            h_weights.resize(kernel_size);
            h_biases.resize(1);
            ...
            d_weights = sycl::buffer<double>(h_weights.data(), sycl::range<1>(kernel_size));
            d_biases = sycl::buffer<double>(h_biases.data(), sycl::range<1>(1));
            std::cout << "Conv1D Layer created: input=" << input_size << ", kernel=" << kernel_size 
                      << ", stride=" << stride << " -> output=" << this->output_size << std::endl;
        }

        h_delta.resize(this->output_size * 400);
...
public:
    ...
    std::vector<double> predict(std::vector<double> inputs, int num_samples, int num_features, int batch_size) {
        std::vector<double> all_predictions(num_samples * layers.back()->output_size);
        sycl::buffer<double> d_all_inputs(inputs.data(), sycl::range<1>(num_samples * num_features));
        ...
        for (int i = 0; i < num_samples; i += batch_size) {
            ...
            sycl::buffer<double> host_batch_preds(all_predictions.data() + i * layers.back()->output_size, sycl::range<1>(current_batch_size * layers.back()->output_size));
    ...
    void train(std::vector<double> inputs, std::vector<double> labels, int num_samples, int num_features, int num_classes, int epochs, int batch_size) {
        std::cout << "Starting training on device: " << q.get_device().get_info<sycl::info::device::name>() << std::endl;

        sycl::buffer<double> d_all_inputs(inputs.data(), sycl::range<1>(num_samples * num_features));
        sycl::buffer<double> d_all_labels(labels.data(), sycl::range<1>(num_samples * num_classes));
        ...
        std::vector<double> predictions = predict(inputs, num_samples, num_features, batch_size);
...
std::vector<std::pair<double, double>> get_normalization_params(std::vector<double> data, int rows, int cols) {
...
void apply_normalization(std::vector<double> data, int rows, int cols, const std::vector<std::pair<double, double>>& params) {
    for (int j = 0; j < cols; ++j) {
...
int main() {
    ...
    auto X_flat = flatten(X_vec);
    auto y_flat = flatten(y_vec);
    ...
    auto test_predictions = nn.predict(X_flat, rows, columns, batch_size);
